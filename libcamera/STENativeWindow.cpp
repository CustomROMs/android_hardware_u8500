...
int STENativeWindow::setCrop(int left, int top, int right, int bottom)
{
    ...
    int error = mPreviewWindow->set_crop(
                        mPreviewWindow,
                        left, top,
                        right, bottom);
    if (OK != error) {
...
int STENativeWindow::setBufferCount(int count)
{
    ...
    int error = mPreviewWindow->set_buffer_count(
...
int STENativeWindow::dequeueBuffer(buffer_handle_t **ppBuffer, int *pStride)
{
    ...
    int stride;
    int error = mPreviewWindow->dequeue_buffer(
                        mPreviewWindow,
                        ppBuffer,
                        &stride);
    if (OK != error) {
...
int STENativeWindow::enqueueBuffer(buffer_handle_t &pBuffer)
{
    ...
    int error = mPreviewWindow->enqueue_buffer(
                        mPreviewWindow,
                        &pBuffer);
    if (OK != error) {
        DBGT_CRITICAL("error = %d", error);
...
int STENativeWindow::cancelBuffer(buffer_handle_t &pBuffer)
{
    ...
    int error = mPreviewWindow->cancel_buffer(
                        mPreviewWindow,
                        &pBuffer);
    if (OK != error) {
        DBGT_CRITICAL("error = %d", error);
...
int STENativeWindow::lockBuffer(buffer_handle_t &pBuffer)
{
    ...
    int error = mPreviewWindow->lock_buffer(
                        mPreviewWindow,
                        &pBuffer);
    if (OK != error) {
        DBGT_CRITICAL("error = %d", error);
