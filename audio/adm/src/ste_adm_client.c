...
ste_adm_res_t STE_ADM_EXPORT ste_adm_set_DHA_mode(void)
{
     return 0;
}
...
ste_adm_res_t STE_ADM_EXPORT ste_adm_set_toplevel_force_map(const char *toplevel_device, const char *actual_device)
{
    if (!toplevel_device || !actual_device) {
        return STE_ADM_RES_INVALID_PARAMETER;
    }
    ...
    ste_adm_res_t result = adm_connect_and_exchange_cmd(&cmd);
    if(result != STE_ADM_RES_OK){
        LOG_ERR_("ste_adm_set_toplevel_force_map error %d, device name %s", result, actual_device);
    }
