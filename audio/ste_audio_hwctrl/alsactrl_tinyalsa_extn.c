...
int pcm_prepare(struct pcm *pcm)
{
	...
	if (ret)
		ALOGE("Cannot prepare channel, error = %s", strerror(errno));
	return ret;
}
									...
									int value, unsigned int idx)
{
	...
	case SNDRV_CTL_ELEM_TYPE_ENUMERATED:
		...
		if (value > no_items - 1) {
			ALOGE("ERROR: Enum-index %d outside bounds!\n", (int)value);
			return -EINVAL;
		}
...
int scan_for_next_card(int cardno, char *card_id, int size)
{
	...
	if (!cardinfo) {
		ALOGE("Error: Malloc failed unable to allocate memory to cardinfo");
		return -1;
	}
	int fd, ret;
	char control[30];
	sprintf(control, "/dev/snd/controlC%i", cardno);
	ALOGW("Trying file name = %s with cardno = %d", control, cardno);
	fd = open(control, O_RDONLY);
	if (fd > 0) {
		ret = get_card_info(fd, cardinfo);
		ALOGW("cardno = %d,cardid = %s", cardno, cardinfo->id);
		if (ret < 0)
			close(fd);
		if (card_id)
...
int scan_for_next_device(int card_fd, int *device)
{
	...
	if (ioctl(card_fd, SNDRV_CTL_IOCTL_PCM_NEXT_DEVICE, device) < 0) {
		ALOGE("SNDRV_CTL_IOCTL_PCM_NEXT_DEVICE failed err = %s", strerror(errno));
		return -errno;
	}
	return 0;
}
...
int get_card_info(int card_fd, struct snd_ctl_card_info *cardinfo)
{
	...
	if (ioctl(card_fd, SNDRV_CTL_IOCTL_CARD_INFO, cardinfo) < 0) {
		ALOGE("SNDRV_CTL_IOCTL_CARD_INFO failed with err = %s", strerror(errno));
		return -errno;
	}
	return 0;
}
...
int get_pcm_info(int pcm_fd, struct snd_pcm_info *pcminfo)
{
	...
	if (ioctl(pcm_fd, SNDRV_CTL_IOCTL_PCM_INFO, pcminfo) < 0) {
		ALOGE("SNDRV_CTL_IOCTL_PCM_INFO failed with err = %s", strerror(errno));
		return -errno;
	}
	return 0;
}
...
/*
	...
	for (cardno = 0; cardno < MAX_SND_CARD_NBR; cardno++) {
		...
		ALOGW("scan_for_next_card fd:%d, id:%s, cardname:%s\n", ret, id, cardname);
		if (ret > 0) {
			ALOGW("Close fd:%d\n",ret);
			close(ret);
